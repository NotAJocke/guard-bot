...
const command: SlashCommand = {
	...
	async exec(interaction: ChatInputCommandInteraction) {
		...
		if (subcommand === "add") {
			...
			Database.addNoteToMember(
				(memberOption as GuildMember).id,
				interaction.guildId!,
				noteOption
			);
			...
			Database.removeNotesFromMember(
				(memberOption as GuildMember).id,
				interaction.guildId!
			);
			interaction.reply({
				content: "Notes supprimées avec succès",
				ephemeral: true,
			});
		} else if (subcommand === "get") {
			const notes = await Database.getNotesOfMember(
				(memberOption as GuildMember).id,
				interaction.guildId!
			);

			interaction.reply({
				content: notes ? "```" + notes + "```" : "Aucune note",
				ephemeral: true,
			});
		}
	},
